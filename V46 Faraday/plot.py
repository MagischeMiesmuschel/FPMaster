...
plt.plot(lam**2,theta_norm_undot,'o')
plt.grid()
plt.xlabel(r'$\lambda^2/\mu m^2$')
plt.ylabel(r'$\theta_{norm}/rad/mm$')
plt.savefig('build/undotiert.pdf')
plt.clf()

plt.plot(lam**2,theta_norm_leichtdot,'go',label="leicht dotiert")
plt.plot(lam**2,theta_norm_hochdot,'ro',label='hoch dotiert')
plt.plot(lam[7]**2,theta_norm_leichtdot[7],'ko',label='vernachlässigter Datenpunkt')
plt.plot(lam_fit,Gerade(lam_fit,*params_leicht),'g--')
plt.plot(lam_fit,Gerade(lam_fit,*params_hoch),'r--')
plt.grid()
plt.legend()
plt.xlabel(r'$\lambda^2/\mu m^2$')
plt.ylabel(r'$\Delta\theta_{norm}/rad/mm$')
plt.savefig('build/deltaTheta.pdf')
plt.clf()
